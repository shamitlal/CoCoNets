...
def pca_1d(emb,keep):
    ...
    emb = emb + EPS
    #emb is B x C x H x W
    emb = emb.cpu().detach().numpy() #this is B x  C

    B, C = np.shape(emb)
    # if np.isnan(emb).any():
    #     out_img = torch.zeros(B, keep, H, W)

    pixelskd = np.reshape(emb, (B, C))
    P = PCA(keep)
    P.fit(pixelskd)
    pixels3d = P.transform(pixelskd)
    out_img = np.reshape(pixels3d, [B,keep]).astype(np.float32)
    return torch.from_numpy(out_img).cuda()
...
class Summ_writer(object):
    ...
    def summ_pca3d_implicit(self, name, xyz, feats_xyzs, dimensions, vox,concat=None,only_return= False):
        if self.save_this:
            ...
            Z, Y, X = dimensions
            feats_xyzs =feats_xyzs.reshape(B*N,hyp.feat3d_dim)

            inbounds = vox.get_inbounds(xyz, Z, Y, X, already_mem=True)
            ...
            voxels = torch.zeros(B*Z*Y*X, hyp.feat3d_dim, device=torch.device('cuda')).float()
            voxels[vox_inds.long(),:] = feats_xyzs
            # st()
            # zero out the singularity
            voxels[base.long()] = torch.zeros_like(voxels[base.long()])
            voxels = voxels.reshape(B, Z, Y, X, hyp.feat3d_dim)
            voxels = voxels.permute(0,4,1,2,3)
            # self.summ_feat('feat3d_implicit/pca', voxels, pca=True)
            # st()
            return self.summ_feat('feat3d_implicit/pca_concat', voxels, pca=True, only_return = only_return)


    def summ_pca3d_implicit_valid(self, name, xyz, feats_xyzs, dimensions, vox,concat=None,only_return= False):
        if self.save_this:
            B, N, C = list(xyz.shape)
            assert(C==3)
            pca_dim = 3
            ...
            vox_inds = base + z * dim2 + y * dim3 + x
            voxels = torch.zeros(B*Z*Y*X, pca_dim, device=torch.device('cuda')).float()
            bin_voxels = torch.zeros(B*Z*Y*X, 1, device=torch.device('cuda')).float()
            # st()
            feats_xyzs_pca = pca_1d(feats_xyzs,pca_dim).unsqueeze(0)
            feats_xyzs_pca = utils.basic.normalize(feats_xyzs_pca).squeeze(0) - 0.5

            voxels[vox_inds.long(),:] = feats_xyzs_pca
            bin_voxels[vox_inds.long(),:] = 1.0
            # st()
            # zero out the singularity
            voxels[base.long()] = torch.zeros_like(voxels[base.long()])
            bin_voxels[base.long()] = 0.0
            voxels = voxels.reshape(B, Z, Y, X, pca_dim)
            voxels = voxels.permute(0,4,1,2,3)

            bin_voxels= bin_voxels.reshape(B, Z, Y, X, 1)
            bin_voxels = bin_voxels.permute(0,4,1,2,3)

            # self.summ_feat('feat3d_implicit/pca', voxels, pca=True)
            # st()
            feat = voxels
            if feat.ndim==5: # B x C x D x H x W
                feat = utils.basic.reduce_masked_mean(feat,bin_voxels,dim=3)
            B, C, D, W = list(feat.shape)
            return self.summ_rgb(name, feat, only_return=only_return)

        # if concat is not None:
        #     voxels = torch.cat([voxels[:1],concat],dim=1)
            # st()
        # return voxels

    def summ_pca2d_implicit(self, name, xyzs, feats_xyzs, pix_T_camX, dimensions,size, only_return=False):
        B, N, D = list(xyzs.shape)
        assert(D==3)
        z_all = xyzs[:,:,2]
        xy_all = utils.geom.apply_pix_T_cam(pix_T_camX, xyzs)
        feats_xyzs_all = feats_xyzs
        (H,W) = size
        feats = torch.zeros(B, hyp.feat3d_dim, H, W, dtype=torch.float32, device=torch.device('cuda'))
        valids = torch.zeros(B, 1, H, W, dtype=torch.float32, device=torch.device('cuda'))
        # st()
        for b in list(range(B)):
            depth = torch.zeros(H*W, dtype=torch.float32, device=torch.device('cuda'))
            xy,feats_xyz,z = (xy_all[b], feats_xyzs_all[b], z_all[b])
            xy = torch.round(xy).long()
            feat = torch.zeros(H*W,hyp.feat3d_dim, dtype=torch.float32, device=torch.device('cuda'))
            ...
            feat = feat.reshape(H,W,hyp.feat3d_dim)
            feat = feat.permute(2,0,1)
            feats[b] = feat
            valids[b] = valid
        # st()
        feat_pca = get_feat_pca(feats, valids)
    ...
    def summ_pca3d_implicit_valid_temporal(self, name, xyz, feats_xyzs, dimensions, vox,concat=None,only_return= False):
        if self.save_this:
            xyz = torch.cat(xyz,dim=0)
            feats_xyzs = torch.cat(feats_xyzs,dim=0)
            ...
            if feat.ndim==5: # B x C x D x H x W
                feat = utils.basic.reduce_masked_mean(feat,bin_voxels,dim=3)
            B, C, D, W = list(feat.shape)
            self.summ_rgbs(name, torch.unbind(feat.unsqueeze(1),dim=0))
    ...
    def summ_pca2d_implicit_temporal(self, name, xyzs, feats_xyzs, pix_T_camX, dimensions,size, only_return=False):
        xyzs = xyzs.squeeze(0)
        pix_T_camX = pix_T_camX.squeeze(0)
        feats_xyzs = torch.cat(feats_xyzs,dim=0)
        B, N, D = list(xyzs.shape)
        assert(D==3)
        z_all = xyzs[:,:,2]
        ...
        feats_xy_pca = pca_1d(feats,3).unsqueeze(0)
        feats_xy_pca = utils.basic.normalize(feats_xy_pca).squeeze(0) - 0.5        
        feats_xy_pca = feats_xy_pca.reshape([B,H,W,3])
        feats_xy_pca = feats_xy_pca.permute(0,3,1,2)
        # feat_pca = get_feat_pca(feats, valids)
        self.summ_rgbs(name, torch.unbind(feats_xy_pca.unsqueeze(1),dim=0))
    ...
    def summ_pointcloud_on_rgb(self, name, xyz_camX, rgb_camX, pix_T_camX, only_return=False):      
        '''
		xyz_camX -> B,N,3
		rgb_camX -> B,3,H,W
		pix_T_camX -> B,4,4
        ...
        xyz_camX_proj = utils.geom.apply_pix_T_cam(pix_T_camX, xyz_camX)
        xyz_camX_proj = xyz_camX_proj[0]
        rgb_camX_ = rgb_camX[0].clone()
        _, H, W = rgb_camX_.shape
        xyz_camX_proj[:, 0] = torch.clamp(xyz_camX_proj[:, 0], 0, W-1)
        xyz_camX_proj[:, 1] = torch.clamp(xyz_camX_proj[:, 1], 0, H-1)
        xyz_camX_proj = xyz_camX_proj.long()

        rgb_camX_[:, xyz_camX_proj[:, 1], xyz_camX_proj[:, 0]] = 0.5
        if only_return:
            return rgb_camX_
        self.summ_rgb(name, rgb_camX_.unsqueeze(0))

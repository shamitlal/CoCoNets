...
class Vox_util(object):
    def __init__(self, Z, Y, X, set_name, scene_centroid, assert_cube=True):
        ...
        if self.set_name=='test':
            ...
            self.XMIN = hyp.XMIN_train
            self.XMAX = hyp.XMAX_train
            self.YMIN = hyp.YMIN_train
            self.YMAX = hyp.YMAX_train
            self.ZMIN = hyp.ZMIN_train
            self.ZMAX = hyp.ZMAX_train
    ...
    def apply_pixX_T_memR_to_voxR(self, pix_T_camX, camX_T_camR, voxR, D, H, W, noise_amount=0.0, grid_z_vec=None, logspace_slices=False):
        # mats are B x 4 x 4
    ...
    def get_sampled_inbound_points(self, xyz_camX, num_pts = hyp.num_tsdf_to_sample):
        res = 100
        inbounds = self.get_inbounds(xyz_camX, res, res, res, already_mem=False)
        occupied_points_list = []
        for b in range(xyz_camX.shape[0]):
            inbounds_b = inbounds[b]
            inbound_idxs = torch.where(inbounds_b > 0)[0] # These are indices into points
            try:
                selected_inbound_idxs = np.random.choice(np.arange(inbound_idxs.shape[0]), num_pts, replace=False) # These are indices into inbound_idxs
            except Exception as e:
                selected_inbound_idxs = np.random.choice(np.arange(inbound_idxs.shape[0]), num_pts, replace=True) # These are indices into inbound_idxs
            selected_inbound_idxs = inbound_idxs[selected_inbound_idxs] # These are indices into points
            occupied_coords_b = xyz_camX[b, selected_inbound_idxs] 
            occupied_points_list.append(occupied_coords_b)
        occupied_points = torch.stack(occupied_points_list)
        return occupied_points

    def get_free_and_occupied_points(self, xyz_camX, num_pts = 10000):
        '''
        xyz_camX is the dense pointcloud of shape BxNx3
        '''
        res = 100
        xyz_memX = self.Ref2Mem(xyz_camX, res, res, res)
        inbounds = self.get_inbounds(xyz_memX, res, res, res, already_mem=True)
        occ_memX = self.get_occupancy(xyz_memX, res, res, res)

        occupied_points_list = []
        free_points_list = []
        for b in range(xyz_memX.shape[0]):
            # Find num_pts unoccupied points
            inbounds_b = inbounds[b]
            inbound_idxs = torch.where(inbounds_b > 0)[0] # These are indices into points
            selected_inbound_idxs = np.random.choice(np.arange(inbound_idxs.shape[0]), num_pts, replace=False) # These are indices into inbound_idxs
            selected_inbound_idxs = inbound_idxs[selected_inbound_idxs] # These are indices into points
            ...
            # Now find num_pts unoccupied points
            occ_memX_b = occ_memX[b, 0]
            unocc_idxs = torch.where(occ_memX_b < 1)
            unocc_z, unocc_y, unocc_x = unocc_idxs
            # We can move points in same voxel and still remain unoccpied
            unocc_x = unocc_x + torch.rand(unocc_x.shape[0]).cuda()
            unocc_y = unocc_y + torch.rand(unocc_y.shape[0]).cuda()
            unocc_z = unocc_z + torch.rand(unocc_z.shape[0]).cuda()
            unoccupied_memcoords_b = torch.stack([unocc_x, unocc_y, unocc_z], dim=-1)
            unoccupied_memcoords_b = unoccupied_memcoords_b[np.random.choice(np.arange(unoccupied_memcoords_b.shape[0]), num_pts, replace=False)]
            unoccupied_coords_b = self.Mem2Ref(unoccupied_memcoords_b.unsqueeze(0), res, res, res)
            free_points_list.append(unoccupied_coords_b[0])

        occupied_points = torch.stack(occupied_points_list)
        free_points = torch.stack(free_points_list)
        return occupied_points, free_points

    # def get_free_and_occupied_points_better(self, xyz_camXs, camX0_T_camX):
    #     '''
    #     xyz_camX - B,S,N,3
    #     camX0_T_camX - B,S,4,4
    #     '''
    #     res = 100
    #     num_pts = 200
    #     B = xyz_camXs.shape[0]
    ...
    #     camX0_T_camX_ = __p(camX0_T_camX)
        
    #     xyz_camX0 = __u(utils.geom.apply_4x4(camX0_T_camX_, xyz_camXs_))
    #     B,S,N,_ = xyz_camX0.shape
    #     xyz_camX0 = xyz_camX0.reshape(B, N*S, -1) # create dense merged pcd 
    #     inbounds = self.get_inbounds(xyz_camX0, res, res, res, already_mem=False)


    #     xyz_memXs = self.Ref2Mem(xyz_camXs_, res, res, res)
    #     occ_memXs = self.get_occupancy(xyz_memXs, res, res, res)
    #     free_memXs = self.get_freespace(xyz_camXs_, occ_memXs)
    #     freeX0_ = self.apply_4x4_to_vox(camX0_T_camX_, free_memXs)
    #     freeX0 = __u(freeX0_)
    #     freeX0 = torch.clamp(torch.sum(freeX0, dim=1), 0, 1)

    #     occupied_points_list = []
    #     free_points_list = []

    #     for b in range(xyz_camX0.shape[0]):
    #         # Find num_pts unoccupied points
    #         inbounds_b = inbounds[b]
    #         inbound_idxs = torch.where(inbounds_b > 0)[0] # These are indices into points
    #         selected_inbound_idxs = np.random.choice(np.arange(inbound_idxs.shape[0]), num_pts) # These are indices into inbound_idxs
    #         selected_inbound_idxs = inbound_idxs[selected_inbound_idxs] # These are indices into points
    #         occupied_coords_b = xyz_camX0[b, selected_inbound_idxs] 
    #         occupied_points_list.append(occupied_coords_b)

    #         # Now find num_pts unoccupied points
    #         occ_memX_b = occ_memX[b, 0]
    #         unocc_idxs = torch.where(occ_memX_b < 1)
    #         unocc_x, unocc_y, unocc_z = unocc_idxs
    #         # We can move points in same voxel and still remain unoccpied
    #         unocc_x = unocc_x + torch.rand(unocc_x.shape[0]).cuda()
    #         unocc_y = unocc_y + torch.rand(unocc_y.shape[0]).cuda()
    #         unocc_z = unocc_z + torch.rand(unocc_z.shape[0]).cuda()
    #         unoccupied_memcoords_b = torch.stack([unocc_x, unocc_y, unocc_z], dim=-1)
    #         unoccupied_memcoords_b = unoccupied_memcoords_b[np.random.choice(np.arange(unoccupied_memcoords_b.shape[0]), num_pts)]
    #         unoccupied_coords_b = self.Mem2Ref(unoccupied_memcoords_b.unsqueeze(0), res, res, res)
    #         free_points_list.append(unoccupied_coords_b[0])
    ...
    def assemble_padded_obj_masklist_xyz(self, lrtlist, scorelist, Z, Y, X, xyz_ref_, coeff=1.0, additive_coeff=0.0):
        ...
        # xyz_ref__ = self.Mem2Ref(xyz_mem_, Z, Y, X)
        # # this is B*N x V x 3

        xyz_ref_ = xyz_ref_.reshape(B*N, -1, 3)
        # this is B*N x V x 3
        ...
        masklist = masklist.reshape(B, N, -1)
        # print(masklist.shape)
        # print(scorelist.shape)
        masklist = masklist*scorelist.view(B, N, 1)
        return masklist

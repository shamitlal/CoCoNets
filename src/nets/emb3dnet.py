...
class Emb3dNet(nn.Module):
    ...
    def compute_ce_loss(self, B, C, Z, Y, X, emb_e_vec_all, emb_g_vec_all, valid_vec_all, mod='', do_vis=False, summ_writer=None, feats_implicit_g=None, feats_implicit_e=None, feats_implicit_n=None):
        if hyp.do_localdecoder or hyp.point_contrast_og:
            implicit_ml_num_sample_points = feats_implicit_e.shape[-1]
            emb_e_vec = feats_implicit_e.permute(0,2,1).reshape(B*implicit_ml_num_sample_points, C)
            emb_g_vec = feats_implicit_g.permute(0,2,1).reshape(B*implicit_ml_num_sample_points, C)
            emb_n_vec = feats_implicit_n.permute(0,2,1).reshape(B*implicit_ml_num_sample_points, C)
            ...
            emb_e_vec = emb_e_vec.view(B*self.num_samples, C) # torch.Size([4, 64])
    ...
    def forward(self, emb_e, emb_g, vis_e, vis_g, feats_implicit_g=None, feats_implicit_e=None, feats_implicit_n=None, summ_writer=None,shapes= None):
        total_loss = torch.tensor(0.0).cuda()

        # st()
        if not hyp.point_contrast_og:
        ...
        # st()
        ce_loss = self.compute_ce_loss(B, C, D, H, W, emb_e_vec, emb_g_vec , vis_g_vec, 'g', False, summ_writer, feats_implicit_g, feats_implicit_e, feats_implicit_n)
        total_loss = utils.misc.add_loss('emb3d/emb_ce_loss', total_loss, ce_loss, hyp.emb3d_ce_coeff, summ_writer)

        if not (hyp.do_localdecoder or hyp.point_contrast_og):

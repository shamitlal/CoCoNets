...
class DetNet(nn.Module):
    def __init__(self):
        ...
        # self.conv1 = torch.nn.Conv3d(in_channels=hyp.feat3D_dim, out_channels=self.pred_dim, kernel_size=3, stride=1, padding=1).cuda()
        print(self.net)
    ...
    def forward(self,
        ...
        corners = utils_geom.get_xyzlist_from_lrtlist(lrtlist_g, legacy_format=True) # corners is B x N x 8 x 3, last dim in xyz order
        ...
        centers_g = utils_geom.get_clist_from_lrtlist(lrtlist_g)
        ...
        lengths_g = utils_geom.get_lenlist_from_lrtlist(lrtlist_g) # B x N x 3

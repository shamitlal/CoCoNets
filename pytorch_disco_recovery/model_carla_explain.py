...
class CARLA_EXPLAIN(Model):
    # torch.set_default_tensor_type('torch.cuda.FloatTensor')
    def initialize_model(self):
        print("------ INITIALIZING MODEL OBJECTS ------")
        self.model = CarlaExplainModel()
    ...
    def go(self):
        ...
        if hyp.do_test:
            ...
            all_track_ious_3d = np.zeros([hyp.max_iters, hyp.S_test], np.float32)
            all_track_ious_2d = np.zeros([hyp.max_iters, hyp.S_test], np.float32)
            all_track_ious_pers = np.zeros([hyp.max_iters, hyp.S_test], np.float32)
                ...
                if log_this or set_do_backprop or hyp.do_test:
                    ...
                    for rep in list(range(repeats)):
                        ...
                        if hyp.do_test and (not returned_early):
                            ...
                            # zoom_proposal_maps_3d = results['all_zoom_proposal_maps_3d']
                            # all_zoom_proposal_maps_3d[test_count] = zoom_proposal_maps_3d
                            # zoom_proposal_maps_2d = results['all_zoom_proposal_maps_2d']
                            # all_zoom_proposal_maps_2d[test_count] = zoom_proposal_maps_2d
                            ...
                            if not hyp.do_flow and eval_track:
                                ...
                                track_ious_3d = results['all_track_ious_3d']
                                all_track_ious_3d[test_count] = track_ious_3d
                                track_ious_2d = results['all_track_ious_2d']
                                all_track_ious_2d[test_count] = track_ious_2d
                                track_ious_pers = results['all_track_ious_pers']
                                all_track_ious_pers[test_count] = track_ious_pers
                            ...
                            if not hyp.do_flow and eval_track:
                                ...
                                mean_track_ious_3d = np.mean(all_track_ious_3d[:test_count], axis=0)
                                print('mean_track_ious_3d', mean_track_ious_3d)

                                mean_track_ious_2d = np.mean(all_track_ious_2d[:test_count], axis=0)
                                print('mean_track_ious_2d', mean_track_ious_2d)
                                
                                mean_track_ious_pers = np.mean(all_track_ious_pers[:test_count], axis=0)
                                print('mean_track_ious_pers', mean_track_ious_pers)
...
class CarlaExplainModel(nn.Module):
    def __init__(self):
        super(CarlaExplainModel, self).__init__()
    ...
    def prepare_common_tensors(self, feed):
        ...
        # self.xyz_camXs = self.xyz_camXs[:,:,::2]
        ...
        while not all_ok:
            ...
            occ_memXA0 = self.vox_util.voxelize_xyz(self.xyz_camXAs[:,0], 80, 80, 80)
            occ_memXAE = self.vox_util.voxelize_xyz(self.xyz_camXAs[:,-1], 80, 80, 80)
    ...
    def run_explain(self, feed):
        ...
        if not have_feats:# and use_feat_cache:
            with torch.no_grad():
                ...
                self.weak_memXAI_all = []
                self.occ_memXAI_all = []
                self.occg_memXAI_all = []
                occrel_memXAI_all = []
                ...
                for I in list(range(self.S)):
                    ...
                    self.occg_memXAI_all.append(occ_memXAI_g)
                    ...
                    weak_memXAI =  self.crop_feat(F.interpolate(feat_memXAI_input, scale_factor=0.5, mode='trilinear'))
                    ...
                    self.weak_memXAI_all.append(weak_memXAI)
        ...
        self.summ_writer.summ_occs('3D_feats/occg_memXAI', self.occg_memXAI_all)
        self.summ_writer.summ_oneds('3D_feats/occrel_memXAI', occrel_memXAI_all, bev=True, norm=False, max_along_y=True)
        ...
        if not have_medians:
            ...
            weak_memXAI_all_np = (torch.stack(self.weak_memXAI_all).detach().cpu().reshape(self.S, -1)).numpy()
            weak_memXAI_median_np = np.median(weak_memXAI_all_np, axis=0)
            self.weak_memXAI_median = torch.from_numpy(weak_memXAI_median_np).float().reshape(1, -1, Z2, Y2, X2).cuda()
        ...
        for I in list(range(self.S)):
            ...
            occ_memXAI = self.occ_memXAI_all[I]
            feat_memXAI = self.feat_memXAI_all[I]
            ...
            # diff = torch.norm(feat_memXAI - self.feat_memXAI_median, dim=1, keepdim=True) * occrel_memXAI
        ...
        super_iters = 4
        for super_iter in list(range(super_iters)):
            print('-'*100)
            ...
            if best_size > 3:
                ...
                # here i need code from model_carla_zoom i think
        ...
        # super_lrtlist is in camX coords
        # and i can evaluate it easily

        # super_lrtlist is B x S x N x 19
        # self.lrt_camXs is B x S x 19
        # 
        
        # gt_lrt_camXs_ = __p(self.lrt_camXs).unsqueeze(1)
        # # this is S x 1 x 19
        ...
        all_ious_3d = np.zeros([self.S])
        all_ious_bev = np.zeros([self.S])
        all_ious_2d = np.zeros([self.S])
        all_ious_valid = np.zeros([self.S])

        print('super_lrtlist', super_lrtlist.shape)
        print('super_scorelist', super_scorelist.shape)
        sN = list(super_scorelist.shape)[2]
        for n in list(range(sN)):
            lrtlist_e, lrtlist_g, scorelist_e, scorelist_g = utils_eval.drop_invalid_lrts(
                super_lrtlist[:,:,n], self.lrt_camXs, super_scorelist[:,:,n], self.score_s)
            max_iou_3d = 0.0
            if torch.sum(scorelist_g) > 0 and torch.sum(scorelist_e) > 0:
                all_ious_valid[s] = 1.0

                iou_3d, iou_bev = utils_geom.get_iou_from_corresponded_lrtlists(
                    lrtlist_e, lrtlist_g)

                iou_3d = iou_3d.detach().cpu().numpy()
                iou_bev = iou_bev.detach().cpu().numpy()

                boxlist_e = utils_geom.get_boxlist2d_from_lrtlist(self.pix_T_cams[:,0], lrtlist_e).detach().cpu().numpy()
                boxlist_g = utils_geom.get_boxlist2d_from_lrtlist(self.pix_T_cams[:,0], lrtlist_g).detach().cpu().numpy()

                iou_2d = utils_box.boxlist_2d_iou(boxlist_e, boxlist_g)
                
                # iou_2d = np.zeros([self.S])
                # for s in list(range(self.S)):
                #     # iou_2d_ = utils_box.boxlist_2d_iou(boxlist_e[:,s], boxlist_g[:,s])
                #     print('iou_2d_', iou_2d_.shape)
                #     iou_2d[s] = iou_2d_
                #     # iou_ = utils_eval.get_mAP_from_2d_boxlists(boxlist_e, scorelist_e, boxlist_g, iou_thresholds)
                
                
                print('iou_3d', iou_3d.shape)
                print('iou_bev', iou_bev.shape)
                print('iou_2d', iou_2d.shape)
                iou_3d = np.squeeze(iou_3d[0])
                iou_bev = np.squeeze(iou_bev[0])
                iou_2d = np.squeeze(iou_2d[0])
                if np.sum(iou_3d) > max_iou_3d:
                    max_iou_3d = np.sum(iou_3d)
                    all_ious_3d = iou_3d
                    all_ious_bev = iou_bev
                    all_ious_2d = iou_2d

                # print('iou_3d', iou_3d, iou_3d.shape)
                # print('iou_bev', iou_bev, iou_bev.shape)
                
            #     all_ious_3d[n]
                
            #     ious_3d, ious_2d = utils_eval.get_mAP_from_lrtlist(lrtlist_e, scorelist_e, lrtlist_g, iou_thresholds)
            #     all_maps_3d[s] = maps_3d
            #     all_maps_2d[s] = maps_2d
            #     boxlist_e = utils_geom.get_boxlist2d_from_lrtlist(self.pix_T_cams[:,s], lrtlist_e)
            #     boxlist_g = utils_geom.get_boxlist2d_from_lrtlist(self.pix_T_cams[:,s], lrtlist_g)
        ...
        results['all_track_ious_3d'] = all_ious_3d
        results['all_track_ious_2d'] = all_ious_bev
        results['all_track_ious_pers'] = all_ious_2d

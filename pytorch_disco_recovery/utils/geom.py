...
# def find_closest_indices(xyz_mem_intesects,xyz_memR_g_int):
#     # xyz_mem_intesects_normalized = utils.basic.l2_normalize(xyz_mem_intesects.to(torch.float),dim=2)
#     # xyz_memR_g_int_normalized = utils.basic.l2_normalize(xyz_memR_g_int.to(torch.float),dim=2)
#     # xyz_memR_g_int_normalized_p = xyz_memR_g_int_normalized.permute(0,2,1)
#     st()
#     # xyz_scores = torch.matmul(xyz_mem_intesects_normalized,xyz_memR_g_int_normalized_p)
#     # torch.argmax(xyz_scores)
#     print("hello")

def find_closest(xyz_g,xyz_e):
    # st()
    xyz_g_normed = utils.basic.l2_normalize(xyz_g,2)
    xyz_e_normed = utils.basic.l2_normalize(xyz_e,2)
    indexes_e = torch.randperm(xyz_e.shape[1])
    indexes_e = indexes_e[::20].to(torch.long)
    # st()
    xyz_e_normed_sub_sampled = xyz_e_normed[:,indexes_e,:]
    
    indexes_e = torch.stack([indexes_e,indexes_e],dim=0)

    xyz_g_normed_p = xyz_g_normed.permute(0,2,1)

    xyz_scores = torch.matmul(xyz_e_normed_sub_sampled,xyz_g_normed_p)
    xyz_maxScoresIndices = torch.max(xyz_scores,dim=2)
    xyz_maxScores = xyz_maxScoresIndices.values
    indexes_g = xyz_maxScoresIndices.indices

    filtered_indexes_g = []
    filtered_indexes_e = []

    for i in range(xyz_e.shape[0]):
        xyz_indices = torch.where(xyz_maxScores[i:i+1] > 0.995)
        f_indexes_e = indexes_e[i:i+1,xyz_indices[1]]
        f_indexes_g = indexes_g[i:i+1,xyz_indices[1]]
        filtered_indexes_g.append(f_indexes_g)
        filtered_indexes_e.append(f_indexes_e)
        # st()
    minimum_indices = min([i.shape[1] for i in filtered_indexes_e])

    filtered_indexes_e = [ i[:, :minimum_indices] for i in filtered_indexes_e]
    filtered_indexes_g = [ i[:, :minimum_indices] for i in filtered_indexes_g]
    # st()

    filtered_indexes_e = torch.cat(filtered_indexes_e,dim=0)
    filtered_indexes_g = torch.cat(filtered_indexes_g,dim=0)

    # xyz_e_normed_tmp  = xyz_e_normed[0,filtered_indexes_e[0][:50]].unsqueeze(-1)
    # xyz_g_normed_tmp  = xyz_g_normed[0,filtered_indexes_g[0][:50]].unsqueeze(1)
    # torch.matmul(xyz_g_normed_tmp,xyz_e_normed_tmp)
    # st()    
    return filtered_indexes_e, filtered_indexes_g
...
def get_colors_for_pointcloud(xyz_camX, rgb_camX, pix_T_camX):
    xyz_camX_proj = apply_pix_T_cam(pix_T_camX, xyz_camX)
    _, _, H, W = rgb_camX.shape
    ...
    xyz_colors_camX_list = []
    for b in range(rgb_camX.shape[0]):
        xyz_colors_camX_b = rgb_camX[b, :, xyz_camX_proj[b,:,1], xyz_camX_proj[b,:,0]]
        xyz_colors_camX_list.append(xyz_colors_camX_b)
    xyz_colors_camX = torch.stack(xyz_colors_camX_list, dim=0)
    return xyz_colors_camX, xyz_camX_proj

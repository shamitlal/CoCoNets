...
# the idea of this mode is to overfit to a few examples and prove to myself that i can generate reloc outputs

class CARLA_RELOC(Model):
    # torch.set_default_tensor_type('torch.cuda.FloatTensor')
    def initialize_model(self):
        print("------ INITIALIZING MODEL OBJECTS ------")
        self.model = CarlaRelocModel()
...
class CarlaRelocModel(nn.Module):
    def __init__(self):
        super(CarlaRelocModel, self).__init__()
    ...
    def prepare_common_tensors(self, feed):
        ...
        mask_memX00 = self.vox_util.assemble_padded_obj_masklist(
            self.full_lrtlist_camX0s[:,0], self.full_scorelist_s[:,0], self.Z2, self.Y2, self.X2, coeff=1.0)
        mask_memX00 = torch.sum(mask_memX00, dim=1).clamp(0, 1)
        self.mask_memX00 = self.crop_feat(mask_memX00)

        # for b in list(range(self.B)):
        #     # if torch.sum(mask_memX00[b]) < 16:
        #     if torch.sum(mask_memX00[b]) < 16:
        #         print('returning early, since there are zero objects inbound at frame0')
        #         return False
        if torch.sum(self.mask_memX00) < 16:
            print('returning early, since there are zero objects inbound at frame0')
            return False

        
        self.summ_writer.summ_oned('3D_feats/mask_memX00', self.mask_memX00, bev=True, norm=False)
    ...
    def run_test(self, feed):
        ...
        self.rgb_memX00 = self.vox_util.unproject_rgb_to_mem(
            self.rgb_camXs[:,0], self.Z, self.Y, self.X, self.pix_T_cams[:,0])
        self.rgb_memX00 = self.vox_util.apply_4x4_to_vox(self.camX0s_T_camXs[:,0], self.rgb_memX00)
        self.occ_memX00 = self.vox_util.voxelize_xyz(self.xyz_camX0s[:,0], self.Z, self.Y, self.X)
        self.occ_halfmemX00 = self.vox_util.voxelize_xyz(self.xyz_camX0s[:,0], self.Z2, self.Y2, self.X2)

        self.rgb_memX01 = self.vox_util.unproject_rgb_to_mem(
            self.rgb_camXs[:,1], self.Z, self.Y, self.X, self.pix_T_cams[:,1])
        self.rgb_memX01 = self.vox_util.apply_4x4_to_vox(self.camX0s_T_camXs[:,1], self.rgb_memX01)
        self.occ_memX01 = self.vox_util.voxelize_xyz(self.xyz_camX0s[:,1], self.Z, self.Y, self.X)
        self.occ_halfmemX01 = self.vox_util.voxelize_xyz(self.xyz_camX0s[:,1], self.Z2, self.Y2, self.X2)

        
        feat_memX00_input = torch.cat([
            self.occ_memX00,
            self.rgb_memX00*self.occ_memX00,
        ], dim=1)
        feat_memX01_input = torch.cat([
            self.occ_memX01,
            self.rgb_memX01*self.occ_memX01,
        ], dim=1)

        if hyp.do_feat3D and hyp.do_occ:
            _, feat_memX00, _ = self.featnet3D(feat_memX00_input)
            _, feat_memX01, _ = self.featnet3D(feat_memX01_input)

            feat_memX00_input_ = F.interpolate(feat_memX00_input, scale_factor=0.5, mode='trilinear')
            feat_memX01_input_ = F.interpolate(feat_memX01_input, scale_factor=0.5, mode='trilinear')
            feat_memX00_input_ = self.crop_feat(feat_memX00_input_)
            feat_memX01_input_ = self.crop_feat(feat_memX01_input_)

            _, occ_memX00_pred = self.occnet(feat_memX00)
            _, occ_memX01_pred = self.occnet(feat_memX01)
            occ_memX00 = F.sigmoid(occ_memX00_pred)
            occ_memX01 = F.sigmoid(occ_memX01_pred)

            feat_memX00_shuf = feat_memX00.permute(0, 2, 3, 4, 1)
            feat_memX01_shuf = feat_memX01.permute(0, 2, 3, 4, 1)
            occ_memX00_shuf = occ_memX00.permute(0, 2, 3, 4, 1)
            occ_memX01_shuf = occ_memX01.permute(0, 2, 3, 4, 1)
            feat_memX00_shuf = feat_memX00.reshape(-1, hyp.feat3D_dim)
            feat_memX01_shuf = feat_memX01.reshape(-1, hyp.feat3D_dim)
            occ_memX00_shuf = occ_memX00.reshape(-1)
            occ_memX01_shuf = occ_memX01.reshape(-1)

            if torch.sum(occ_memX00_shuf*occ_memX01_shuf) < 32:
                print('not enough voxels occ')
                return total_loss, results, True

            # self.summ_writer.summ_feats('3D_feats/feat_memX0_orig_input', [feat_memX00_input, feat_memX01_input], pca=True)
            self.summ_writer.summ_feats('3D_feats/feat_memX0_input', [feat_memX00_input_, feat_memX01_input_], pca=True)
        ...
        self.summ_writer.summ_occ('proposals/mot_pred', mot_memX00_pred)
        # mot_memX00_pred[mot_memX00_pred < 0.5] = 0.0
        # self.summ_writer.summ_occ('proposals/mot_pred_clean', mot_memX00_pred)
        
        # # blur it up
        # mot_memX00_pred = F.interpolate(mot_memX00_pred, scale_factor=0.5, mode='trilinear')
        # mot_memX00_pred = F.interpolate(mot_memX00_pred, scale_factor=2.0, mode='trilinear')
        # self.summ_writer.summ_occ('proposals/mot_pred_blurred', mot_memX00_pred)

        mot_memX00_pad = self.pad_feat(mot_memX00_pred)
        
        self.K = 4

        if True:
            boxes_image, boxlist_memX0, scorelist_e, tidlist, connlist = utils_misc.get_boxes_from_flow_mag(mot_memX00_pad.squeeze(1), self.K)
            ...
            print('connlist', connlist.shape)
            self.summ_writer.summ_occ('proposals/connlist', torch.sum(connlist, dim=1, keepdims=True).clamp(0, 1))
        else:
            center_e = _nms(mot_memX00_pred)
            self.summ_writer.summ_occ('proposals/centers', center_e)
            scorelist_e, xyzlist_crop_e = _topk(center_e, K=self.K)
            Z_crop, Y_crop, X_crop = self.crop_guess
            crop_vec = torch.from_numpy(np.reshape(np.array([X_crop, Y_crop, Z_crop]), (1, 1, 3))).float().cuda()
            xyzlist_mem_e = xyzlist_crop_e + crop_vec
            xyzlist_cam_e = self.vox_util.Mem2Ref(xyzlist_mem_e, self.Z2, self.Y2, self.X2)
            centroids = xyzlist_cam_e.clone()
            # this is B x K x 3

            # let's turn these centroids into boxes
            # the mean length is something like this, in meters:
            ...
            rots = rots * 0.0 # for now, set rotation to zero
            ...
            tidlist = torch.arange(self.K).reshape(1, self.K).long().cuda()
        ...
        lrtlist_e, lrtlist_g, scorelist_e, scorelist_g = utils_eval.drop_invalid_lrts(
            lrtlist_cam, self.full_lrtlist_camX0s[:,0], scorelist_e, self.full_scorelist_s[:,0])
        ...
        t_noise[:,0] = 0.0
        t_noise[:,:,1] = 0.0 # set the Y part to be zero, so we only reloc on the road
        ...
        for tr in list(range(self.num_tries)):
            ...
            relocd_feat_memX0 = bkg_memX0 * (1.0 - obj_mask_halfmemX0) + obj_mask_halfmemX0 * obj_memX0
            self.summ_writer.summ_feat('3D_feats/relocd_feat_memX0_%d' % tr, relocd_feat_memX0, pca=True)

...
class CARLA_FORECAST(Model):
    ...
    def initialize_model(self):
        ...
        self.model = CarlaForecastModel().to(self.device)
...
class CarlaForecastModel(nn.Module):
    def __init__(self):
        super(CarlaForecastModel, self).__init__()
        in_dim = 1
        if hyp.do_feat3D:
    ...
    def prepare_common_tensors(self, feed):
        ...
        if set_data_format=='traj' or set_data_format=='simpletraj':
            ...
            # self.occ_memR0 = self.vox_util.voxelize_xyz(self.xyz_camR0, self.Z, self.Y, self.X)
            # self.occ_memX0 = self.vox_util.voxelize_xyz(self.xyz_camX0, self.Z, self.Y, self.X)
    ...
    def train_forecaster(self, feed):
        ...
        # sy = self.ZY*self.vox_size_Y
        # sx = self.ZX*self.vox_size_X
        # make life easier, by keeping the same bounds as the regular mem
        sz = hyp.ZMAX-hyp.ZMIN
        sy = hyp.YMAX-hyp.YMIN
        sx = hyp.XMAX-hyp.XMIN
        
        # print('sz, sy, sx', sz, sy, sx)
        ...
        # self.summ_writer.summ_traj_on_occ('3D_inputs/obj_traj_memX0',
        #                                   self.obj_clist_camX0s,
        #                                   self.occ_memX0, 
        #                                   self.vox_util, 
        #                                   already_mem=False,
        #                                   sigma=1)
        # self.summ_writer.summ_traj_on_occ('3D_inputs/obj_traj_memR0',
        #                                   self.obj_clist_camR0s,
        #                                   self.occ_memR0, 
        #                                   self.vox_util, 
        #                                   already_mem=False,
        #                                   sigma=1)
        ...
        if hyp.do_motionreg:
            motionreg_loss, clists_0_e = self.motionregnet(
                feat_0,
                clist_0,
            ...
            if self.summ_writer.save_this:
                print('clists_0_e[0,0]', clists_0_e[0,0].detach().cpu().numpy())
                print('clist_0[0]', clist_0[0].detach().cpu().numpy())
                o = []
                for k in list(range(hyp.motionreg_num_slots)):
                    o.append(utils_improc.preprocess_color(
                        self.summ_writer.summ_traj_on_occ(
                            ...
                            occ_0[0:1],
                            self.vox_util,
                            traj_g=(clist_0[0:1] if (k==0) else None),
                            already_mem=True,
                            only_return=True,
                            sigma=1)))
                summ_vis = torch.max(torch.stack(o, dim=0), dim=0)[0]
                self.summ_writer.summ_rgb('motionreg/clists_0_eg', summ_vis)

                # self.summ_writer.summ_traj_on_occ('motionreg/clist_0_g',
                #                                   clist_0[0:1],
                #                                   occ_0[0:1], 
                #                                   self.vox_util, 
                #                                   already_mem=True,
                ...
                clists_0_e_ = clists_0_e.reshape(self.B, hyp.motionreg_num_slots*self.S, 3)
                clists_camR0_e_ = self.vox_util.Zoom2Ref(clists_0_e_, lrt_0, self.ZZ, self.ZY, self.ZX)

                clists_camR0_e = clists_camR0_e_.reshape(self.B, hyp.motionreg_num_slots, self.S, 3)

                dists = torch.norm(clists_camR0_e - self.obj_clist_camR0s.unsqueeze(1), dim=3)
                # this is B x K x S
            ...
            # self.summ_writer.summ_traj_on_occ('3D_inputs/obj_traj_e',
            #                                   clist_0_e,
            #                                   occ_0, 
            #                                   self.vox_util, 
            #                                   already_mem=True,
        ...
        # xyz0 = self.obj_clist_camR0s[:,0]
        # xyz1 = self.obj_clist_camR0s[:,1]
        # # these are B x 3
        ...
        # rot0 = utils_geom.eye_3x3(self.B)
        ...
        # tra = torch.from_numpy(mid_xyz).float().cuda().repeat(self.B, 1)
        # center_T_noyaw = utils_geom.merge_rt(rot0, tra)

        # noyaw_T_camR0 = utils_basic.matmul3(center_T_noyaw, noyaw_T_zero, zero_T_camR0)
        # noyaw_T_camR0s = noyaw_T_camR0.unsqueeze(1).repeat(1, self.S, 1, 1)

        # print('obj_clist_camR0s', self.obj_clist_camR0s.shape)
        # clist_camNY0s = utils_geom.apply_4x4(noyaw_T_camR0, self.obj_clist_camR0s)
        # print('clist_camNY0s', clist_camNY0s.shape)
        
        # xyz_camNY0s = __u(utils_geom.apply_4x4(__p(noyaw_T_camR0s), __p(self.xyz_camR0s)))
        # occ_NY0s_, _ = self.vox_util.voxelize_near_xyz(__p(xyz_camNY0s), __p(clist_camNY0s),
        #                                                self.ZZ, self.ZY, self.ZX,
        #                                                sz=sz, sy=sy, sx=sx)
        # occ_NY0s = __u(occ_NY0s_)
        
        # # this is B x S x 1 x ZZ x ZY x ZX
        # self.summ_writer.summ_occs('3D_inputs/occ_NY0s', torch.unbind(occ_NY0s, dim=1))
        # # occ_NY0s = __u(self.vox_util.voxelize_xyz(__p(xyz_camNY0s), ZZ, ZY, ZX)) 
        # self.summ_writer.summ_traj_on_occ('3D_inputs/obj_traj',
        #                                   clist_camNY0s,
        #                                   torch.max(occ_NY0s, dim=1)[0],
        #                                   self.vox_util, 
        #                                   already_mem=False,
        #                                   sigma=2)

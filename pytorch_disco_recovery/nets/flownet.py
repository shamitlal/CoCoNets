...
class FlowNet(nn.Module):
    def __init__(self):
        ...
        assert(self.num_scales==1) # do not touch scales, unless you adjust the masking
        ...
        self.grid = torch.stack([grid_z, grid_y, grid_x], dim=1) - int(self.heatmap_size/2)
    ...
    def generate_flow(self, feat0, feat1, sc):
        ...
        if self.debug:
            # print('cc:')
            print(cc.shape)
            # print('top left corner:', cc[0,:,:,:,0,0,0])
            # print('middle left corner:', cc[0,:,:,:,15,15,15])
        cc = cc.view(B, 1, self.heatmap_size**3, D, H, W)
        # cc = F.relu(cc) # zero out negative matches, as relja recommended
        cc = torch.nn.functional.softmax(cc/0.07, dim=2)
        flow = torch.sum(cc * self.grid, dim=2)

        # self.grid = self.grid.reshape(1, 3, -1, 1, 1, 1)
    ...
    def forward(self, feat0, feat1, flow_g=None, mask_g=None, summ_writer=None):
        ...
        if flow_g is not None:
            ...
            nonzero_mask = ((torch.sum(torch.abs(flow_g), axis=1, keepdim=True) > 0.01).float())*mask_g*mask
            ...
            if summ_writer is not None:
                ...
                summ_writer.summ_3D_flow('flow/flow_e_%.2f' % sc, flow_total, clip=clip)
                summ_writer.summ_3D_flow('flow/flow_g_%.2f' % sc, flow_g, clip=clip)
                summ_writer.summ_oned('flow/mask_%.2f' % sc, mask, bev=True, norm=False)
                summ_writer.summ_feat('flow/flow_e_pca_%.2f' % sc, flow_total, pca=True)

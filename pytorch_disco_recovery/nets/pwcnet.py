...
class Network(torch.nn.Module):
	def __init__(self):
		...
		self.load_state_dict(torch.load('./nets/network-' + arguments_strModel + '.pytorch'))
	# end
...
class PWCNet(torch.nn.Module): #building graph and estimate
	...
	def forward(self, tensorFirst, tensorSecond):
		...
		assert(tensorFirst.shape == tensorSecond.shape)
		if tensorFirst.ndim == 3: #no batch dim
			tensorFirst = tensorFirst.unsqueeze(0) # B x C x H x W
			tensorSecond = tensorSecond.unsqueeze(0)

		_, _, intHeight, intWidth = tensorFirst.shape 
		...
		# these should be in size B x C x H x W
		...
		intPreprocessedWidth = 1024
		intPreprocessedHeight = 448

		tensorPreprocessedFirst = torch.nn.functional.interpolate(input=tensorPreprocessedFirst, size=(intPreprocessedHeight, intPreprocessedWidth), mode='bilinear', align_corners=False)
		tensorPreprocessedSecond = torch.nn.functional.interpolate(input=tensorPreprocessedSecond, size=(intPreprocessedHeight, intPreprocessedWidth), mode='bilinear', align_corners=False)

		tensorFlow = 20.0 * torch.nn.functional.interpolate(input=self.moduleNetwork(tensorPreprocessedFirst, tensorPreprocessedSecond), size=(intHeight, intWidth), mode='bilinear', align_corners=False)
...
if __name__ == '__main__':
	...
	tensorOutput = pwcnet(tensorFirst, tensorSecond)
	tensorOutput = tensorOutput.cpu()
	# tensorOutput = estimate(tensorFirst, tensorSecond)

	import flow_vis
	
	flow_uv = tensorOutput.numpy().transpose(1, 2, 0)
	flow_color = flow_vis.flow_to_color(flow_uv, convert_to_bgr=False)

	plt.imsave('./flo.png', flow_color)

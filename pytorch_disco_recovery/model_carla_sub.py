...
# class grad_integrator_2d(nn.Module):
#     def __init__(self, w):
#         super(grad_integrator_2d, self).__init__()
#         self.w = w
#         self.filterx = torch.cuda.FloatTensor(1,1,1,self.w).fill_(1)
#         self.filtery = torch.cuda.FloatTensor(1,1,self.w,1).fill_(1)
#         self.filterx = torch.autograd.Variable(self.filterx, requires_grad=False)
#         self.filtery = torch.autograd.Variable(self.filtery, requires_grad=False)
#         self.filterx.cuda()
#         self.filtery.cuda()
...
class spatial_integrator_3d(nn.Module):
    def __init__(self,w):
        super(spatial_integrator_3d, self).__init__()
        self.w = w
        self.filterz = torch.cuda.FloatTensor(1,1,self.w,1,1).fill_(1)
        self.filtery = torch.cuda.FloatTensor(1,1,1,self.w,1).fill_(1)
        self.filterx = torch.cuda.FloatTensor(1,1,1,1,self.w).fill_(1)
        self.filterz = torch.autograd.Variable(self.filterz, requires_grad=False)
        self.filtery = torch.autograd.Variable(self.filtery, requires_grad=False)
        self.filterx = torch.autograd.Variable(self.filterx, requires_grad=False)
        self.filterz.cuda()
        self.filtery.cuda()
    ...
    def forward(self, input_diffgrid):
        print('self.w', self.w)
        print('spatial integ input', input_diffgrid.shape)
        
        fullz = F.conv_transpose3d(input_diffgrid[:,2].unsqueeze(1), self.filterz, stride=1, padding=0)
        fully = F.conv_transpose3d(input_diffgrid[:,1].unsqueeze(1), self.filtery, stride=1, padding=0)
        fullx = F.conv_transpose3d(input_diffgrid[:,0].unsqueeze(1), self.filterx, stride=1, padding=0)
        print('fullz', fullz.shape)
        
        output_grid = torch.cat((fullx[:,:,0:self.w,0:self.w,0:self.w],
                                 fully[:,:,0:self.w,0:self.w,0:self.w],
                                 fullz[:,:,0:self.w,0:self.w,0:self.w]),1)
        return output_grid
...
# the idea of this mode is to overfit to a few examples and prove to myself that i can generate sub outputs

class CARLA_SUB(Model):
    # torch.set_default_tensor_type('torch.cuda.FloatTensor')
    def initialize_model(self):
        print("------ INITIALIZING MODEL OBJECTS ------")
        self.model = CarlaSubModel()
...
class CarlaSubModel(nn.Module):
    def __init__(self):
        super(CarlaSubModel, self).__init__()
        ...
        if hyp.do_flow:
            ...
            # self.integrator_2d = grad_integrator_2d(int(hyp.Z/2)-self.crop_guess[0]) # 
            # self.integrator_3d = spatial_integrator_3d(int(hyp.Z/2)-self.crop_guess[0])
            self.integrator_3d = spatial_integrator_3d(int(hyp.Z/2)-self.crop_guess[0]*2)
    ...
    def run_sub(self, feed):
        ...
        if False:
            ...
            # how about this as an alternative:
            # for each object, find its max iou with the neighbor frame's boxes

            # full_lrtlist_camX0s is B x S x N x 19
            # full_scorelist_s is B x S x N

            new_scorelist_s = torch.zeros_like(self.full_scorelist_s)
            for n in list(range(self.N)):
                # lrt_camX0s = self.full_lrtlist_camX0s[:,:,n] # B x S x 19
                # scores = self.full_scorelist_s[:,:,n] # B x S
                ...
                # drop_invalid_lrts(lrtlist_e, lrtlist_g, scorelist_e, scorelist_g)

                # lrtlist_e, lrtlist_g, scorelist_e, scorelist_g = utils_eval.drop_invalid_lrts(
                #     lrt_camXs[s:s+1], full_lrtlist_camXs_[s:s+1], scorelist_s_[s:s+1], full_scorelist_s_[s:s+1])
                    ...
                    for b in list(range(self.B)):
                        if score[b] > 0.5:
                            ...
                            for i in list(range(self.N)):
                                if self.full_scorelist_s[b,s1,i] > 0.5:
                            ...
                            # self.full_scorelist_s[:,s0,n] = torch.from_numpy(np.squeeze(np.max(ious))).cuda()
                            # self.full_scorelist_s[:,s0,n] = torch.from_numpy(np.squeeze(np.max(ious))).cuda()
                            
                            # new_scorelist_s[b,s0,n] = float(np.max(ious))
        ...
        if hyp.do_use_cache:
            ...
            if os.path.isfile(feat_cache_fn):
                print('found cache at %s; we will use this' % feat_cache_fn)
                ...
                # occ_memXAI_vis = torch.from_numpy(cache['occ_memXAI_vis']).unbind(1)
                # mask_memXAI_vis = torch.from_numpy(cache['mask_memXAI_vis']).unbind(1)
                ...
                print('loaded feats; time %.2f' % (time.time() - start_time))
            else:
        ...
        if not have_feats:
            ...
            for I in list(range(self.S)):
                ...
                mask_memXAI = self.vox_util.assemble_padded_obj_masklist(
                    self.full_lrtlist_camXAs[:,I], self.full_scorelist_s[:,I], self.Z, self.Y, self.X, coeff=1.0, additive_coeff=0.5)
                mask_memXAI = torch.sum(mask_memXAI, dim=1).clamp(0, 1)
                # mask_full_vis.append(self.summ_writer.summ_occ('', mask_memXAI, only_return=True))
                mask_memXAI = F.interpolate(mask_memXAI, scale_factor=0.5, mode='trilinear')
                ...
                # occ_memXAI = occ_memXAI * occrel_memXAI
                # self.summ_writer.summ_occ('occrel/occrel_occ', F.sigmoid(occ_memX0_pred) * occrel_memX0_pred)
                ...
                # feat_memXAI_input_ = F.interpolate(feat_memXAI_input, scale_factor=0.5, mode='trilinear')
                # feat_memXAI_input_ = self.crop_feat(feat_memXAI_input_)
                # feat_memXAI_input_ = F.interpolate(self.occ_memXAI, scale_factor=0.5, mode='trilinear')
                # feat_memXAI_input_ = self.crop_feat(feat_memXAI_input_)

                # feat_memXAI_vis.append(self.summ_writer.summ_feat('', feat_memXAI, pca=True, only_return=True))
                # feat_memXAI_input_vis.append(self.summ_writer.summ_feat('', feat_memXAI_input, pca=False, only_return=True))
                # feat_memXAI_input_vis.append(self.summ_writer.summ_occ('', feat_memXAI_input_, only_return=True))
                ...
                # mask_memXAI_vis.append(self.summ_writer.summ_occ('', mask_memXAI, only_return=True))
                print('got feats for I=%d; time %.2f' % (I, time.time() - start_time))
            ...
            if hyp.do_use_cache:
                ...
                # save_dict['feat_memXAI_vis'] = torch.stack(feat_memXAI_vis, dim=1).detach().cpu().numpy()
                save_dict['occ_memXAI_vis'] = torch.stack(occ_memXAI_vis, dim=1).detach().cpu().numpy()
                save_dict['occrel_memXAI_vis'] = torch.stack(occrel_memXAI_vis, dim=1).detach().cpu().numpy()
                # save_dict['feat_memXAI_input_vis'] = torch.stack(feat_memXAI_input_vis, dim=1).detach().cpu().numpy()
                # save_dict['mask_memXAI_vis'] = torch.stack(mask_memXAI_vis, dim=1).detach().cpu().numpy()
                np.savez(feat_cache_fn, save_dict=save_dict)
                print('saved feats to %s cache, for next time' % feat_cache_fn)


        # # assuming i can compute flow densely, i should like to see it
        # feat0 = feat0.contiguous()
        # feat1 = feat1.contiguous()
        # cc = self.correlation_sampler(feat0, feat1)
        # print('cc', cc.shape)
        # cc = cc.view(self.B, 1, self.heatmap_size**3, Z2, Y2, X2)
        # print('cc', cc.shape)
        # cc = F.relu(cc) # zero out negative matches, as relja recommended
        # cc = F.softmax(cc/0.07, dim=2)
        # flow = torch.sum(cc * self.offset_grid, dim=2)
        # clip = 3.0
        # self.summ_writer.summ_3D_flow('flow/flow_e', flow, clip=clip)
        ...
        if hyp.do_use_cache:
            ...
            if os.path.isfile(med_cache_fn):
                ...
                print('loaded medians; time %.2f' % (time.time() - start_time))
            else:
        ...
        if not have_medians:
            ...
            # self.summ_writer.summ_occ('3D_feats/occ_memXAI_diff', occ_memXAI_diff)
        ...
        # note i already computed the features
        print('got medians; time %.2f' % (time.time() - start_time))
        ...
        if hyp.do_flow:
            ...
            for s in list(range(self.S-5)):
                ...
                # print('flow05', flow05.shape)
                # dz, dy, dx = utils_basic.gradient3D(flow05)
                # flow2d_grad =  torch.cat([dz, dx], dim=1)
                # print('flow2d_grad', flow2d_grad.shape)
                # flow2d_new = self.integrator_2d(flow2d_grad)
                # print('flow2d_new', flow2d_new.shape)
                # z, x = flow2d_new[:,0], flow2d_new[:,1]
                # flow05_new = torch.stack([z, torch.zeros_like(z), x], dim=1)
                # print('flow05_new', flow05_new.shape)

                print('flow05', flow05.shape)
                dz, dy, dx = utils_basic.gradient3D_(flow05)
                dnew = torch.cat([dx[:,0,:,:,:].unsqueeze(1), dy[:,1,:,:,:].unsqueeze(1), dz[:,2,:,:,:].unsqueeze(1)], dim=1)
                flow05_new = self.integrator_3d(dnew)
                flow05_new[:,0] += flow05[:,0,:,:,0:1]
                flow05_new[:,1] += flow05[:,1,:,0:1,:]
                flow05_new[:,2] += flow05[:,2,0:1,:,:]
                # flow05_new and flow05 are equal except the last index in each spatial dim.

                # import pdb; pdb.set_trace()
                # z = self.integrator_3d(dz)
                # y = self.integrator_3d(dy)
                # x = self.integrator_3d(dx)
                # print('dz', dz.shape)
                # print('z', z.shape)
                # # flow05_new = torch.cat([z, y, x], dim=1)
                # flow05_new = z+y+x
                ...
                flow05_new_vis.append(self.summ_writer.summ_3D_flow('', flow05_new, occ=occ0*(1.0-occ_memXAI_median), clip=clip, only_return=True))
                ...
                consistent_flow05_vis.append(self.summ_writer.summ_3D_flow(
                    '', flow05, occ=mask*occ0*(1.0-occ_memXAI_median), clip=clip, only_return=True))
            ...
            self.summ_writer.summ_rgbs('3D_feats/flow05_new', flow05_new_vis)
        ...
        # self.summ_writer.summ_rgbs('3D_feats/mask_memXAI', mask_memXAI_vis)
        ...
        print('got all diffs; time %.2f' % (time.time() - start_time))
        ...
        if self.summ_writer.save_this:
            ...
            for I in list(range(self.S-5)):
                ...
                mask_memXAI = self.vox_util.assemble_padded_obj_masklist(
                    self.full_lrtlist_camXAs[:,I], self.full_scorelist_s[:,I], self.Z2, self.Y2, self.X2)
                mask_memXAI = torch.sum(mask_memXAI, dim=1).clamp(0, 1)
                mask_memXAI = self.crop_feat(mask_memXAI)
                ...
                # e = self.summ_writer.summ_occ('', mask_memXAI, only_return=True)
        ...
        if hyp.do_use_cache and use_box_cache:
            ...
            if os.path.isfile(box_cache_fn):
                ...
                print('loaded boxes; time %.2f' % (time.time() - start_time))
            else:

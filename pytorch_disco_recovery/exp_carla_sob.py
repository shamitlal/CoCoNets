...
# the idea here is to do some clean bkg sobtraction

current = 'builder'
current = 'trainer'
# current = 'tester'

mod = '"sob00"' # builder; sob start, copied from _sub
mod = '"sob01"' # S = 50; use cache < OOM
mod = '"sob02"' # compute with nograd < ok
mod = '"sob03"' # no_backprop, just to get through
mod = '"sob04"' # init subnet; show diffs
mod = '"sob05"' # train subnet a bit
mod = '"sob06"' # train a while
mod = '"sob07"' # deleted a weird cache
mod = '"sob08"' # 10
mod = '"sob09"' # same but log10
mod = '"sob10"' # stride1; better summs
mod = '"sob11"' # smooth coeff
mod = '"sob12"' # show gif of outs
mod = '"sob13"' # max_along_y
mod = '"sob14"' # measure accuracies
mod = '"sob15"' # make the masks at half res directly
mod = '"sob16"' # call it full_acc; use high-conf stuff
mod = '"sob17"' # show mask summ
mod = '"sob18"' # show sub obj and bkg
mod = '"sob19"' # use 0.1 and 0.9 as threshs, instead of mean+-std
mod = '"sob20"' # summ oned the labels
mod = '"sob21"' # hun
mod = '"sob22"' # use 0.05 and 0.95, instead of 0.1 and 0.9
mod = '"sob23"' # use 0.1 and 0.9
mod = '"sob24"' # ten
mod = '"sob25"' # proper smooth loss
mod = '"sob26"' # do not use feat cache; stride 4
mod = '"sob27"' # nan trick
mod = '"sob28"' # stride 8
mod = '"sob29"' # check for lx>0.01
mod = '"sob30"' # acc=iou
mod = '"sob31"' # area isclose to 0 , iou 0
mod = '"sob32"' # just ensure no nans in inter_p
mod = '"sob33"' # 
mod = '"sob34"' # use 0.05 and 0.95
mod = '"sob35"' # use 0.5 and 0.95
...
groups['carla_sob'] = ['do_carla_sob = True']

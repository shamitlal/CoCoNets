...
class CARLA_CSIAMESE(Model):
    ...
    def initialize_model(self):
        ...
        self.model = CarlaCsiameseModel().to(self.device)
    ...
    def go(self):
        ...
        if hyp.do_test:
            ...
            if hyp.do_export_stats:
                ...
                all_min_confs = np.zeros([hyp.max_iters, hyp.S_test], np.float32)
            ...
            for (set_num,
                ...
                if log_this or set_do_backprop or hyp.do_test:
                    ...
                    if hyp.do_test and (not returned_early):
                        ...
                        mean_min_confs = np.mean(all_min_confs[:test_count], axis=0)
                        print('mean_ious', mean_ious)
                        print('mean_confs', mean_confs)
                        print('mean_min_confs', mean_min_confs)

                        print('data0_time', np.mean(self.model.data0_times))
                        print('data1_time', np.mean(self.model.data1_times))
                        print('feat0_time', np.mean(self.model.feat0_times))
                        print('feat1_time', np.mean(self.model.feat1_times))
                        print('match_time', np.mean(self.model.match_times))
                        print('altmatch_time', np.mean(self.model.altmatch_times))
                        print('conf_time', np.mean(self.model.conf_times))
        ...
        if hyp.do_export_stats:
            ...
            np.save(out_fn, {
                'all_ious': all_ious,
                'all_confs': all_confs,
                'all_min_confs': all_min_confs,
            })
            print('saved %s' % out_fn)
            

class CarlaCsiameseModel(nn.Module):
    def __init__(self):
        super(CarlaCsiameseModel, self).__init__()
        in_dim = 1
        if hyp.do_feat3D:
        ...
        self.min_iou_e_pool = utils_misc.SimplePool(100, version='np')
        self.iou_e_pool = utils_misc.SimplePool(100, version='np')
        self.iou_g_pool = utils_misc.SimplePool(100, version='np')
    ...
    def train_matcher(self, feed):
        ...
        # note that when training the matcher, dt_1 and dr_1 had 0.5 coeffs on them; here we use a wider spread, to mimic test error
        
        # for frame1, place the object somewhere NEAR dt_0
        dt_1 = np.random.uniform(low=-0.5*sz/2.0, high=0.5*sz/2.0, size=[self.B, 3])*1.0
        ...
        if hyp.do_match:
            ...
            corrs, rad_e, obj_loc_halfsearch_e, match_loss = self.matchnet(
                feat_0s_trimmed, # stack of templates
        ...
        if hyp.do_conf:
            ...
            lrt_camR0_e = utils_geom.merge_lrt(obj_len, obj_rt)

            lrt_camR1_e = utils_geom.apply_4x4_to_lrtlist(self.camRs_T_camR0s[:,1], lrt_camR0_e.unsqueeze(1)).squeeze(1)
            lrt_camX1_e = utils_geom.apply_4x4_to_lrtlist(self.camXs_T_camRs[:,1], lrt_camR1_e.unsqueeze(1)).squeeze(1)

            iou = utils_geom.get_iou_from_corresponded_lrtlists(self.lrt_camRs[:,1:2], lrt_camR1_e.unsqueeze(1)).squeeze(1)
            # this is shaped B
            self.summ_writer.summ_scalar('match/iou', torch.mean(iou))

            # # plot
            # self.occ_memX1 = self.vox_util.voxelize_xyz(self.xyz_camXs[0:1,1], self.Z, self.Y, self.X)
            # self.summ_writer.summ_lrtlist_bev(
            #     'match/box_bev', self.occ_memX1,
            #     torch.cat([self.lrt_camXs[0:1,1:2], lrt_camX1_e[0:1].unsqueeze(1)], dim=1),
            #     torch.cat([torch.ones([1,1]).float().cuda(), iou[0:1].reshape(1, 1)], dim=1),
            #     torch.arange(1,3).reshape(1, 2).long().cuda(), # tids
            #     self.vox_util)

            # self.summ_writer.summ_lrtlist(
            #     'match/box_perspective', self.rgb_camXs[0:1,1],
            #     torch.cat([self.lrt_camXs[0:1,1:2], lrt_camX1_e[0:1].unsqueeze(1)], dim=1),
            #     torch.cat([torch.ones([1,1]).float().cuda(), iou[0:1].reshape(1, 1)], dim=1),
            #     torch.arange(1,3).reshape(1, 2).long().cuda(),
            #     self.pix_T_cams[0:1,1])

            ious_e, conf_loss = self.confnet(
                corrs,
                occ_1,
                iou=iou,
                summ_writer=self.summ_writer)
            ...
            iou_e = torch.mean(ious_e, dim=1)
            min_iou_e = torch.min(ious_e, dim=1)[0]
            # ious_e = [iou_e.detach().cpu().numpy() for iou_e in ious_e]
            # iou_e = np.mean(ious_e, axis=1)
            self.iou_e_pool.update(iou_e.detach().cpu().numpy())
            self.min_iou_e_pool.update(min_iou_e.detach().cpu().numpy())
            self.iou_g_pool.update(iou.detach().cpu().numpy())
            min_ious_e = self.min_iou_e_pool.fetch()
            ious_e = self.iou_e_pool.fetch()
            ious_g = self.iou_g_pool.fetch()
            corr_coeff = np.corrcoef(ious_e, ious_g)[0,1]
            self.summ_writer.summ_scalar('conf/corr_coeff', corr_coeff)
            corr_coeff = np.corrcoef(min_ious_e, ious_g)[0,1]
            self.summ_writer.summ_scalar('conf/corr_coeff_min', corr_coeff)
            
            occ_v, lrt_v = self.vox_util.voxelize_near_xyz(self.xyz_camR0s[0:1,1],
                                                           self.obj_clist_camR0s[0:1,1],
                                                           self.Z, self.Y, self.X,
                                                           sz=32, sy=32, sx=32)
            self.summ_writer.summ_lrtlist_bev(
                'conf/box_bev_v_iou_e', occ_v,
                torch.cat([self.lrt_camR0s[0:1,1:2], lrt_camR0_e[0:1].unsqueeze(1)], dim=1),
                torch.cat([torch.ones([1,1]).float().cuda(), iou_e[0:1].reshape(1, 1)], dim=1),
                torch.arange(1,3).reshape(1, 2).long().cuda(), # tids
                self.vox_util,
                lrt=lrt_v[0:1])
            self.summ_writer.summ_lrtlist_bev(
                'conf/box_bev_v_iou_min', occ_v,
                torch.cat([self.lrt_camR0s[0:1,1:2], lrt_camR0_e[0:1].unsqueeze(1)], dim=1),
                torch.cat([torch.ones([1,1]).float().cuda(), min_iou_e[0:1].reshape(1, 1)], dim=1),
                torch.arange(1,3).reshape(1, 2).long().cuda(), # tids
                self.vox_util,
                lrt=lrt_v[0:1])
            self.summ_writer.summ_lrtlist_bev(
                'conf/box_bev_iou_g', occ_v,
                torch.cat([self.lrt_camR0s[0:1,1:2], lrt_camR0_e[0:1].unsqueeze(1)], dim=1),
                torch.cat([torch.ones([1,1]).float().cuda(), iou[0:1].reshape(1, 1)], dim=1),
                torch.arange(1,3).reshape(1, 2).long().cuda(), # tids
                self.vox_util,
    ...
    def train_forecaster(self, feed):
        ...
        sz = hyp.ZMAX-hyp.ZMIN
        sy = hyp.YMAX-hyp.YMIN
    ...
    def track_over_seq(self, feed):
        ...
        self.data0_times.append(data_time)
        ...
        self.feat0_times.append(feat_time)
        ...
        min_confs = torch.zeros([self.B, self.S]).float().cuda()
        ...
        for s in list(range(0, self.S)):
            ...
            occ_1, lrt_1 = self.place_object_at_delta(
                ...
                sz, sy, sx,
                self.vox_util)
            data1_time += time.time() - time_start

            time_start = time.time()
            # featurize
            feat_loss, feat_1, validR1 = self.featnet3D(occ_1)#, self.summ_writer)
            feat1_time += time.time() - time_start
            ...
            time_start = time.time()
            # locate the object
            corrs, rad_e, obj_loc_halfsearch_e, _ = self.matchnet(
                feat_0s_trimmed, # stack of templates
                # self.radlist, 
                feat_1)#, # search region
            ...
            match_time += time.time() - time_start
            # match_time += e_time
            # altmatch_time += match_selftime
            altmatch_time += 0.0

            time_start = time.time()
            confs_, _ = self.confnet(corrs, occ_1)
            mean_conf = torch.mean(confs_, dim=1)
            min_conf = torch.min(confs_, dim=1)[0]
            conf_time += time.time() - time_start
            
            # record our confidence
            confs[:,s] = mean_conf
            min_confs[:,s] = min_conf
            self.conf_pools[s].update(mean_conf.cpu().numpy())
        ...
        print('torch ious', ious.shape)
        results['ious'] = ious
        results['confs'] = confs
        results['min_confs'] = min_confs
        self.summ_writer.summ_scalar('track/mean_iou', np.mean(means))

        self.data1_times.append(data1_time)
        self.feat1_times.append(feat1_time)
        self.match_times.append(match_time)
        self.altmatch_times.append(altmatch_time)
        self.conf_times.append(conf_time)

        # vis boxes in perspective and bev
        if self.include_image_summs and self.summ_writer.save_this:
            self.rgb_camXs = feed["rgb_camXs"]
            visX_e = []
            for s in list(range(self.S)):
                visX_e.append(self.summ_writer.summ_lrtlist(
                    ...
                    # torch.cat([torch.ones([self.B,1]).float().cuda(), confs[:,s:s+1]], dim=1),

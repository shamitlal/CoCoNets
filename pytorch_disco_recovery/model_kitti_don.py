...
class KITTI_DON(Model):
    # torch.set_default_tensor_type('torch.cuda.FloatTensor')
    def initialize_model(self):
        print("------ INITIALIZING MODEL OBJECTS ------")
        self.model = KittiDonModel()
...
class KittiDonModel(nn.Module):
    def __init__(self):
        super(KittiDonModel, self).__init__()
        ...
        if hyp.do_feat2D:
            ...
            for s in range(self.S):
                ...
                for b in list(range(B)):
                    ...
                    # corr_b = torch.matmul(feat_vec_b.cpu(), obj_vec_b.cpu())
                    ...
                    xyzI = utils_basic.argmax3D(heat_b, hard=False, stack=True, cuda=True)
                    # this is med x 3

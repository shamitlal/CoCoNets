...
class KITTI_EGO(Model):
    def initialize_model(self):
        print('------ INITIALIZING MODEL OBJECTS ------')
        self.model = KittiEgoModel()
...
class KittiEgoModel(nn.Module):
    def __init__(self):
        super(KittiEgoModel, self).__init__()
    ...
    def run_train(self, feed):
        ...
        if hyp.do_match:
            ...
            self.summ_writer.summ_occs('ego/occ_rs', occ_rs)
            self.summ_writer.summ_unps('ego/rgb_rs', rgb_rs, occ_rs)
            self.summ_writer.summ_feats('ego/feat_rs', feat_rs, pca=True)
            self.summ_writer.summ_feats('ego/feat_rs_trimmed', feat_rs_trimmed, pca=True)
            ...
            self.summ_writer.summ_occs('ego/occ_mems_0', [occ0_mem0, occ1_memX])
            self.summ_writer.summ_occs('ego/occ_mems_e', [occ0_mem0, occ1_mem0_e.round()])
            self.summ_writer.summ_occs('ego/occ_mems_g', [occ0_mem0, occ1_mem0_g.round()])
    ...
    def run_sfm(self, feed):
        ...
        # self.summ_writer.summ_scalar('unscaled_ego/mean_epe', mean_epe)
        ...
        # self.summ_writer.summ_scalar('unscaled_ego/mean_epe_bev', mean_epe_bev)

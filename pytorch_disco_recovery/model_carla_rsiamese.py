...
class CARLA_RSIAMESE(Model):
    ...
    def initialize_model(self):
        ...
        self.model = CarlaRsiameseModel().to(self.device)
...
class CarlaRsiameseModel(nn.Module):
    def __init__(self):
        super(CarlaRsiameseModel, self).__init__()
        if hyp.do_feat3D:
    ...
    def train_matcher(self, feed):
        ...
        dr_1 = dr_0 + torch.from_numpy(rand_r).float().cuda()*0.5
        # dr_1 = dr_0.clone()
        ...
        if hyp.do_match:
            ...
            corr, rad_e, obj_loc_search_e, match_loss = self.matchnet(
    ...
    def track_over_seq(self, feed):
        ...
        for s in list(range(0, self.S)):
            ...
            if hyp.do_match:
                corr, rad_e, obj_loc_halfsearch_e, match_loss = self.matchnet(
                    feat_0s_trimmed, # list of templates
                    self.radlist, 
                    feat_1, # search region

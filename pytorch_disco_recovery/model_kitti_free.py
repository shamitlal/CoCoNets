...
def track_proposal(B, S, 
    ...
    rgb_memXII = orig_vox_util.unproject_rgb_to_mem(
        rgb_camXs[:,I], int(Z_zoom/2), int(Y_zoom/2), int(X_zoom/2), pix_T_cams[:,I])
    rgb_memXAI = orig_vox_util.apply_4x4_to_vox(camXAs_T_camXs[:,I], rgb_memXII)
    occ_memXAI = orig_vox_util.voxelize_xyz(xyz_camXAs[:,I], int(Z_zoom/2), int(Y_zoom/2), int(X_zoom/2))
    feat_memXAI = torch.cat([
        occ_memXAI, rgb_memXAI*occ_memXAI,
    ], dim=1)
    feat_memXAI = cropper(feat_memXAI)
    ...
    for s in range(I+1, S):
        ...
        if torch.sum(inb) == 0:
            ...
            rgb_memXII = new_vox_util.unproject_rgb_to_mem(
                rgb_camXs[:,s], int(Z_zoom/2), int(Y_zoom/2), int(X_zoom/2), pix_T_cams[:,s])
            rgb_memXAI = new_vox_util.apply_4x4_to_vox(camXAs_T_camXs[:,s], rgb_memXII)
            occ_memXAI = new_vox_util.voxelize_xyz(xyz_camXAs[:,s], int(Z_zoom/2), int(Y_zoom/2), int(X_zoom/2))
            feat_memXAI_input = torch.cat([
                occ_memXAI, rgb_memXAI*occ_memXAI,
            ], dim=1)
            feat_memXAI = cropper(feat_memXAI_input)
            occ_memXAI = cropper(occ_memXAI)
...
# the idea of this mode is to overfit to a few examples and prove to myself that i can generate free outputs

class KITTI_FREE(Model):
    # torch.set_default_tensor_type('torch.cuda.FloatTensor')
    def initialize_model(self):
        print("------ INITIALIZING MODEL OBJECTS ------")
        self.model = KittiFreeModel()
    ...
    def go(self):
        ...
        if hyp.do_test:
            ...
            # all_zoom_proposal_maps_3d = np.zeros([hyp.max_iters, hyp.S_test, num_ious], np.float32)
            # all_zoom_proposal_maps_2d = np.zeros([hyp.max_iters, hyp.S_test, num_ious], np.float32)
            test_count = 0
...
class KittiFreeModel(nn.Module):
    def __init__(self):
        super(KittiFreeModel, self).__init__()
    ...
    def run_free(self, feed):
        ...
        for I in list(range(self.S)):
            ...
            self.rgb_memXII = self.vox_util.unproject_rgb_to_mem(
                self.rgb_camXs[:,I], self.Z2, self.Y2, self.X2, self.pix_T_cams[:,I])
            # self.rgb_memXAI = self.vox_util.apply_4x4_to_vox(self.camXAs_T_camXs[:,I], self.rgb_memXII)
            rgb_memXAI = self.vox_util.apply_4x4_to_vox(self.camXAs_T_camXs[:,I], self.rgb_memXII)
            occ_memXAI = self.vox_util.voxelize_xyz(self.xyz_camXAs[:,I], self.Z2, self.Y2, self.X2)

            feat_memXAI = torch.cat([
                occ_memXAI,
                rgb_memXAI*occ_memXAI,
            ], dim=1)

            feat_memXAI = self.crop_feat(feat_memXAI)
            occ_memXAI = self.crop_feat(occ_memXAI)
            occrel_memXAI = torch.ones_like(occ_memXAI)
        ...
        self.summ_writer.summ_feats('3D_feats/feat_memXAI', feat_memXAI_all, pca=True)
        self.summ_writer.summ_occs('3D_feats/occ_memXAI', occ_memXAI_all)
        ...
        for super_iter in list(range(4)):
            ...
            lrtlist_memXAI_all, connlist_memXAI_all, scorelist_all, blue_vis_all = propose_boxes_by_differencing(
                self.K, self.S, occ_memXAI_all, diff_memXAI_all, self.crop_guess,
                self.set_data_name, super_iter)
            ...
            if best_size > 3:
                ...
                if zoom:
                    ...
                    for s in list(range(self.S)):
                        ...
                        rgb_memXII = zoom_vox_util.unproject_rgb_to_mem(
                            self.rgb_camXs[:,s], int(Z_zoom/2), int(Y_zoom/2), int(X_zoom/2), self.pix_T_cams[:,s])
                        rgb_memXAI = zoom_vox_util.apply_4x4_to_vox(self.camXAs_T_camXs[:,s], rgb_memXII)
                        occ_memXAI = zoom_vox_util.voxelize_xyz(self.xyz_camXAs[:,s], int(Z_zoom/2), int(Y_zoom/2), int(X_zoom/2))
                    ...
                    lrtlist_zoomXAI_some, connlist_zoomXAI_some, scorelist_some, blue_vis_some = propose_boxes_by_differencing(
                        ...
                        self.set_data_name, super_iter, summ_writer=self.summ_writer)
                ...
                # here i need code from model_kitti_zoom i think
                ...
                # eliminate the claimed locations from the difference map
                diff_memXAI_all = [diff*(1.0-mask) for (diff, mask) in zip(diff_memXAI_all, mask_memXAI_all)]
